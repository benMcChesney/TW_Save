...
def parse_args():
    ...
    if len(sys.argv) < 2 :
        ...
        args.append(  'skaven_clan_skryre_me.txt' )
        return
    else:
         args.append( sys.argv[1] )

    if len(sys.argv) < 3 :
        print( 'using default args scope - "all"')
        args.append( 'all')
    else:
...
args = parse_args()  
campaign_esf_paths_file = args[0]
scope = args[1]

print(f"loading esf paths with scope {scope} @ '{campaign_esf_paths_file}'" )
...
outputs = [ econ, army, region, characters, battle_result , diplomacy ]
sql_scripts = """
SELECT
  TABLE_NAME
FROM
...
for s in campaign_files:
    ...
    if i >= indexOffset :    
        ...
        try :
            ...
            if scope == 'all' or scope == 'character' :
            ...
            if scope == 'all' or scope == 'battle_result' :
                new_array = []
            ...
            if scope == 'all' or scope == 'diplomacy' :
                new_array = []
